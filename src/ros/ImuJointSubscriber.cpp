  ...
  const std::string &joint_topic)
{
  imu_sub_ = node->create_subscription<sensor_msgs::msg::Imu>(
    ...
    [this](const sensor_msgs::msg::Imu::SharedPtr msg)
    {
      ...
      emit titaStateUpdated(snapshot);
    });
      ...
      TitaState snapshot{};
      {
        ...
        for (size_t i = 0; i < names.size(); ++i)
        {
          auto write_joint = [&](bool left, int index)
          {
            ...
            {
              if (index == 1) side.Real.Pos.joint1 = msg->position[i];
              if (index == 2) side.Real.Pos.joint2 = msg->position[i];
              if (index == 3) side.Real.Pos.joint3 = msg->position[i];
              if (index == 4) side.Real.Pos.Wheel = msg->position[i];
            }
            if (i < msg->velocity.size())
            {
              if (index == 1) side.Real.Vel.joint1 = msg->velocity[i];
              if (index == 2) side.Real.Vel.joint2 = msg->velocity[i];
              if (index == 3) side.Real.Vel.joint3 = msg->velocity[i];
              if (index == 4) side.Real.Vel.Wheel = msg->velocity[i];
            }
            if (i < msg->effort.size())
            {
              if (index == 1) side.Real.Effort.joint1 = msg->effort[i];
              if (index == 2) side.Real.Effort.joint2 = msg->effort[i];
              if (index == 3) side.Real.Effort.joint3 = msg->effort[i];
              if (index == 4) side.Real.Effort.Wheel = msg->effort[i];
            }
          };
